...
class PillarContext3D_dsa(nn.Module):
    def __init__(
        ...
        mlps = self.local_context["mlps"]
        for k in range(len(mlps)):
            mlps[k] = [self.num_bev_features] + mlps[k]
        self.adapt_context = pointnet2_stack_modules.StackSAModuleMSGAdapt(
            radii=self.local_context["pool_radius"],
            deform_radii=self.local_context["deform_radius"],
            nsamples=self.local_context["nsample"],
            mlps=mlps,
            use_xyz=True,
            pool_method=self.local_context["pool_method"],
            pc_range=self.pc_range,
        )

        # UnPool layers
        mlps_decode = self.decode["mlps"]
        for k in range(len(mlps_decode)):
            mlps_decode[k] = [self.in_dim] + mlps_decode[k]
        self.decode = pointnet2_stack_modules.StackSAModuleMSGDecode(
            radii=self.decode["pool_radius"],
            nsamples=self.decode["nsample"],
            mlps=mlps_decode,
            use_xyz=True,
            pool_method=self.decode["pool_method"],
        )

        # self-attention layers to operate on deformed pillars
        self.self_full_fast_attn = SA_block(inplanes=self.in_dim, planes=self.in_dim)
        self.reduce_dim = nn.Sequential(nn.Conv1d(2*self.in_dim, self.in_dim, kernel_size=1),
                                        nn.BatchNorm1d(self.in_dim),
                                        nn.ReLU(inplace=True),
                                        nn.Conv1d(self.in_dim, self.in_dim, kernel_size=1),
                                        nn.BatchNorm1d(self.in_dim),
                                        nn.ReLU(inplace=True)
                                        )
        self.self_attn_ms1 = SA_block(inplanes=2*self.in_dim, planes=2*self.in_dim)
        self.self_attn_ms2 = SA_block(inplanes=2*self.in_dim, planes=2*self.in_dim)
    ...
    def get_context_features(self, batch_size, local_features):
        ...
        for batch_idx in range(batch_size):
            init_idx = batch_idx * self.num_keypoints
            local_feat = local_features[init_idx:init_idx + self.num_keypoints, :].unsqueeze(0)
    ...
    def get_context_image(self, batch_size, keypoints, pillar_center, global_features, coords):
        ...
        context_pillar_features = context_pillar_features.view(batch_size,
                                                               self.num_bev_features * self.nz, self.ny,
                                                               self.nx)
    ...
    def forward(self, batch_dict):
        ...
        pillar_center = torch.zeros_like(coords[:, :3]).float()
        ...
        local_keypoint_feats = local_keypoint_feats.view(batch_size*self.num_keypoints, -1).contiguous()
