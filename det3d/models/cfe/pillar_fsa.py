...
class PillarContext3D_fsa(nn.Module):
    ...
    """
    def __init__(self, num_bev_features, in_dim, grid_size, dropout=0.1):
        super().__init__()
        self.in_dim = in_dim
        self.num_bev_features = num_bev_features

        self.nx, self.ny, self.nz = grid_size
        self.position_enc = PositionalEncoding(self.in_dim, height=grid_size[1], width=grid_size[0])
        self.layer_norm = nn.LayerNorm(self.in_dim, eps=1e-6)

        self.self_attn1 = SA_block(inplanes=self.in_dim, planes=self.in_dim)
        self.self_attn2 = SA_block(inplanes=self.in_dim, planes=self.in_dim)
    ...
    def add_context_to_pillars(self, pillar_features, coords, nx, ny, nz):
        ...
        context_pillar_features = context_pillar_features.view(batch_size, self.num_bev_features * nz, ny, nx)
