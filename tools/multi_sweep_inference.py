...
def remove_low_score_nu(image_anno, thresh):
    ...
    trailer_indices =                get_annotations_indices(1, 0.5, label_preds_, scores_)
    bus_indices = get_annotations_indices(2, 0.5, label_preds_, scores_)
    van_indices =                  get_annotations_indices(3, 0.5, label_preds_, scores_)
    pedestrian_indices =              get_annotations_indices(4, 0.5, label_preds_, scores_)
...
class Processor_ROS:
    ...
    def get_lidar_data(self, input_points: dict):
        ...
        print("points in current frame shape: ", self.current_frame["points"].shape)   
        ...
        if len(self.lidar_deque) == 5:
            ...
            all_pc = np.zeros((4, 0), dtype=float)
            for i in range(len(self.pc_list)):
                tmp_pc = cp.vstack((self.pc_list[i], cp.zeros((1, self.pc_list[i].shape[1]))))
    ...
    if(proc_1.get_lidar_data(input_points)):
        ...
        if scores.size != 0:
            for i in range(scores.size):
                ...
                q = yaw2quaternion(float(dt_box_lidar[i][-1]))
...
if __name__ == "__main__":
    ...
    config_path = 'configs/nusc_centerpoint_pp_02voxel_two_pfn_10sweep_one_head_no_vel.py'
    model_path = 'models/nusc_one_head_no_vel/epoch_400.pth'
    ...
    sub_lidar = rospy.Subscriber(
        sub_lidar_topic[7], PointCloud2, rslidar_callback, queue_size=1, buff_size=2**24)
